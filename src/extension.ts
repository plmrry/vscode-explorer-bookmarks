...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('explorerBookmarks.addBookmark', async (uri: vscode.Uri) => {
			if (uri) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('explorerBookmarks.removeBookmark', async (item: BookmarkItem) => {
			if (item) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('explorerBookmarks.openBookmark', async (uri: vscode.Uri) => {
			if (uri) {
				try {
